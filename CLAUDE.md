...
- **Third-party services / APIs:** **None at runtime — ZERO network calls** (restored in v0.3.0). The v0.2.x auto-update `fetch` of `version.json` was **removed** — ELSA's CSP blocks every request to our domain (confirmed for both `connect-src` and `img-src`), so it never worked there anyway. In its place is a **weekly, network-free update reminder**: a pure local-date check (anchored to Wednesday) that shows a yellow "App may be out of date — Check for update?" banner linking to the setup page. It works identically on and off ELSA because it touches no network. **Keep it this way — do not re-add any runtime network call**; the setup page + version stamp + "check for latest ↗" link are the update path. GitHub Pages is used only for hosting (`version.json` is still published as a static version record, but nothing fetches it). `gh` CLI (installed via Homebrew, authed as `rvanpolen89`) is used for repo/Pages management from the dev machine.
...
- **`tools/build.js`** — the build. Reads `src/`, stamps the version, generates: `dist/HAHNS.html`, `dist/bookmarklet.txt`, `dist/version.json`, `docs/index.html`, `docs/bookmarklet.txt`, `docs/version.json`, `docs/.nojekyll`. **Holds the `VERSION` constant.** Also **renders `CHANGELOG.md` to HTML** (`renderChangelog`, a tiny markdown subset) and injects it into the setup page only (`__CHANGELOG__`). As of v0.3.0 the changelog is **no longer baked into the bookmarklet** (the in-app "What's new" pop-up was removed), which shrank the payload. The only helper placeholder is now `__BUILD__`. **`version.json`** (`{version, build}`) is published as a static version record — nothing fetches it (no network calls).
...
- **Vehicle init (v0.3.2):** `extractVehicle(segments)` reads ELSA's **Vehicle Summary** page into `{vin, year, model, engine, trans}` (any field may be `""`). VIN uses `VIN_RE` (17 chars, no I/O/Q) + `looksVin`; the other four use `vehVal(lines, labelRe, valRe, excludeRe)` — a **heuristic** label→value scan (same line or next), so the field-label regexes likely need tuning against a real-page dump. `VEH_FIELDS` is the shared display order; `vehLoaded(r)`=truthy VIN, `vehMissing(v)`=blank labels. **Gating in `run().scan()`:** until a vehicle is loaded a scan MUST be the summary (a found VIN loads it, **auto-detected** — same Scan button; no VIN → blocked with a `vehNotice` prompt, no specs collected); once loaded, scans collect procedure specs as before. The vehicle bar renders via `vehicleBar(r)` (green "Vehicle loaded" strip, blanks flagged + click-to-edit `.vval`), hidden in `embed` (demo) mode. Vehicle also flows into `plainText`/print/`debugDump` (the dump reports the per-field grab — use it to tune).
...
- **Security / privacy (CRITICAL — this is the product's promise):** **Zero network calls** (restored in v0.3.0 — the v0.2.x auto-update fetch was removed). Nothing phones home, ever. No saving manual content to files. Only the small extracted working list + UI state in sessionStorage (volatile), plus one `localStorage` date string for the weekly reminder. Diagrams stored **by URL reference, not copied**. Shadow DOM isolation. Always preserve this posture; flag any change that would send job/ELSA content anywhere or add **any** runtime network call.
...
## Core features (built)
- **Vehicle init (v0.3.2):** scanning ELSA's **Vehicle Summary** page first loads the vehicle (VIN, Model Year, Model Name, Engine Code, Trans Type) into a green strip at the top of the panel; **required before any procedure specs can be collected**. Blanks are flagged + hand-editable; carried into copy/print. Groundwork for a later feature.
