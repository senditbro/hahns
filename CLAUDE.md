...
- **Fluids & Capacities lookup (v0.3.4):** the fluids section is a **link** (not scanned) that opens a vehicle-matched lookup page (Engine Oil, Engine Coolant, Air Conditioning, Drivetrain — capacities + fluid specs), matched to the loaded vehicle's engine/trans/model/year. Programmer-only PDF→data pipeline per model year; **2011–2026 loaded** (v0.3.8). Older years are harder: **2006–2010** list engines by displacement only (no 4-letter code → engine-code matching can't match; needs a displacement-based fallback in `fluids.html`), and **2000–2005** use a different old 2-column `Component/System | Capacity` layout (needs a second parser path). Both deferred. Parser handles the Unicode `±` tolerance (older PDFs) and skips appended "Maintenance Schedules" sections (v0.3.8).
