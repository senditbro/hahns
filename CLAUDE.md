...
- **Third-party services / APIs:** **None at runtime, with ONE deliberate exception** (v0.2.0+): the **auto-update check** — at most once per day, in the background after render, the app probes GitHub Pages for a newer build **two ways**: (1) a `fetch` of `version.json` (exact version, but blocked by ELSA's `connect-src` CSP — confirmed), and (2) loading a tiny **marker image** `uc/cur/<version>.png` which rides on the more-permissive `img-src` so it can work on ELSA (yes/no, no version string). All requests are referrer-less, cookie-less GETs for public static files on **our own hosting** — **no job/ELSA content, no telemetry**. Failures are silent for the tech (the real reason of both probes, incl. whether CSP actually fired, is recorded in `vwjb_last_update_result_v1` for the diagnostic). Keep the update check scoped to our own hosting domain only. GitHub Pages is used only for hosting. `gh` CLI (installed via Homebrew, authed as `rvanpolen89`) is used for repo/Pages management from the dev machine.
...
- **`tools/build.js`** — the build. Reads `src/`, stamps the version, generates: `dist/HAHNS.html`, `dist/bookmarklet.txt`, `dist/version.json`, `dist/uc/*` + `docs/index.html`, `docs/bookmarklet.txt`, `docs/version.json`, `docs/uc/control.png`, `docs/uc/cur/<version>.png`, `docs/.nojekyll`. **Holds the `VERSION` constant.** `writeMarkers()` wipes `uc/cur/` and writes only the current version's marker each build (older markers vanish → out-of-date apps' markers 404). Also **renders `CHANGELOG.md` to HTML** (`renderChangelog`, a tiny markdown subset) and injects it into the page (`__CHANGELOG__`) and the bookmarklet (helper's `__CHANGELOG_HTML__` as a JS string, `__VERSION__` as the bare version). So the app's "What's new" needs no network — the changelog is baked in. **`version.json`** (`{version, build}`) is what the deployed app fetches for its update check.
...
- **Auto-update check — ELSA blocks the fetch (CONFIRMED), image method is the workaround:** On a real ELSA page (host `www.vwhub.com`), the v0.2.1 `securitypolicyviolation` listener confirmed `csp:true` for `connect-src` — ELSA's CSP **definitively blocks** the `version.json` fetch (not a network fluke). So **v0.2.2 adds a second probe: a marker image** (`uc/cur/<version>.png`) loaded via `img-src`, which ELSA is far more likely to allow (it loads diagram images). Logic: a "control" image (`uc/control.png`, always present) distinguishes blocked/offline from a genuine 404; if control loads but the version marker 404s → "update available" → banner (no version number on ELSA). The diagnostic's `image` block records `controlLoaded`, `markerLoaded`, `imgCsp`, `outcome`. **Whether ELSA's `img-src` allows our images is NOT yet verified on a real ELSA page** (verified only via mocked preview tests) — that's the open question for the v0.2.2 real-ELSA test. GitHub Pages serves with permissive CORS, so CORS is never the blocker. The version stamp + manual "check for latest ↗" link remain the guaranteed fallback.
